...
def role_can_access_operation(role, user_id, operation):
    ...
    operation_company_id = to_int(operation.get("empresa_id"))
    actor_company_id = current_user_company_id()

    if normalized_role == ROLE_GLOBAL:
        return True

    if operation_company_id > 0 and actor_company_id > 0 and operation_company_id != actor_company_id:
        return False
...
def ensure_clients_extra_columns(cursor, db):
    ...
    indexes = {
        row["INDEX_NAME"]: {
            ...
            "columns": str(row.get("columns_joined") or ""),
        }
        for row in cursor.fetchall()
    }

    for legacy_unique_name in ("cpf", "uq_clientes_cpf"):
        index_meta = indexes.get(legacy_unique_name)
        if index_meta and index_meta["non_unique"] == 0 and index_meta["columns"] == "cpf":
            cursor.execute(f"ALTER TABLE clientes DROP INDEX `{legacy_unique_name}`")
            changed = True

    scoped_unique = indexes.get("uq_clientes_empresa_vendedor_cpf")
    if not scoped_unique or scoped_unique["non_unique"] != 0 or scoped_unique["columns"] != "empresa_id,vendedor_id,cpf":
        cursor.execute(
            """
            ALTER TABLE clientes
            ADD UNIQUE INDEX uq_clientes_empresa_vendedor_cpf (empresa_id, vendedor_id, cpf)
            """
        )
        changed = True
...
def register_operation_status_history(
    ...
    operation_row = cursor.fetchone() or {}
    company_id = to_int(operation_row.get("empresa_id")) or None
...
def collect_operation_notification_recipients(
    ...
    company_id = to_int(operation.get("empresa_id"))
    ...
    if include_admins:
        cursor.execute(
            ...
            WHERE UPPER(role) IN ('ADMIN', 'GLOBAL')
              AND (%s = 0 OR empresa_id = %s OR UPPER(role) = 'GLOBAL')
            """
            ,
    ...
    if include_product_digitadores:
        ...
        if digitador_roles:
            ...
            cursor.execute(
                ...
                """,
                tuple([*digitador_roles, company_id, company_id]),
            )
            for row in cursor.fetchall() or []:
                user_id = to_int(row.get("id"))
                if user_id > 0:
...
def insert_operation_notifications(
    ...
    rows = []
    normalized_company_id = to_int(company_id) or None
...
def fetch_client_record_by_cpf(cursor, cpf, company_id=None, vendedor_id=None, exclude_client_id=None):
    conditions = ["c.cpf = %s"]
    params = [cpf]

    if company_id is not None:
        conditions.append("c.empresa_id = %s")
        params.append(company_id)
...
def resolve_dashboard_goal(cursor, year, month, vendedor_id, company_id):
    if vendedor_id:
        cursor.execute(
            ...
            WHERE year=%s AND month=%s AND vendedor_id=%s AND empresa_id=%s
            LIMIT 1
            """,
            (year, month, vendedor_id, company_id),
        )
        row = cursor.fetchone()
        if row:
    ...
    cursor.execute(
        ...
        SELECT target
        FROM dashboard_goals
        WHERE year=%s AND month=%s AND vendedor_id=0 AND empresa_id=%s
        LIMIT 1
        """,
...
def create_client():
    ...
    try:
        ...
        existing_client = fetch_client_record_by_cpf(
            cursor,
            cpf,
        ...
        if existing_client:
            return jsonify({
                "error": "CPF ja cadastrado para este vendedor",
                "code": "CPF_ALREADY_EXISTS_FOR_SELLER",
        ...
        if "empresa_id" in column_meta:
            ...
            insert_values = [seller_company_id, *insert_values]
        ...
        if "Duplicate entry" in message and "cpf" in message.lower():
            duplicate_client = fetch_client_record_by_cpf(
                cursor,
                cpf,
                company_id=seller_company_id,
                vendedor_id=vendedor_id,
...
def search_global():
    ...
    try:
        ...
        apply_company_scope(role, base_where, params, "c.empresa_id")
...
def create_operation_comment(operation_id):
    ...
    cursor.execute(
        """
        INSERT INTO operation_comments (empresa_id, operation_id, author_id, message)
        VALUES (%s, %s, %s, %s)
        """,
        (operation.get("empresa_id"), operation_id, user_id, message),
    )
...
def update_client(client_id):
    ...
    try:
        ...
        existing_vendedor_id = to_int(existing_client.get("vendedor_id"))

        duplicate_client = fetch_client_record_by_cpf(
            cursor,
            cpf,
            company_id=existing_company_id,
            vendedor_id=existing_vendedor_id,
        ...
        if "Duplicate entry" in message and "cpf" in message.lower():
            return jsonify({"error": "CPF ja cadastrado para este vendedor"}), 409
        ...
        if current_status in FINAL_OPERATION_STATUSES:
            ...
            if "vendedor_id" in data:
                if not is_admin_like_role(role):
                    cursor.close()
                    db.close()
                    return jsonify({"error": "Somente admin/global pode alterar o vendedor da proposta"}), 403
...
def get_operations_stats():
    ...
    cursor.execute(
        ...
        FROM operacoes o
        WHERE {date_filter}
          {"AND o.empresa_id = %s" if role != ROLE_GLOBAL else ""}
        """,
        tuple(
            [
                *PIPELINE_ACTIVE_STATUSES_WITH_LEGACY,
                *([] if role == ROLE_GLOBAL else [current_user_company_id()]),
            ]
        ),
    )
...
def get_dashboard_summary():
    ...
    try:
        ...
        if is_admin_like_role(role):
            ...
            if role_product_params:
                ...
                cursor.execute(
                    ...
                    ORDER BY u.nome ASC
                    """,
                    tuple([actor_company_id, *role_product_params]),
                )
                vendors = cursor.fetchall()
            else:
                cursor.execute(
        ...
        goal_company_id = actor_company_id
        if selected_vendor_id:
            cursor.execute(
                """
            ...
            goal_company_id = to_int(selected_vendor.get("empresa_id")) or actor_company_id
